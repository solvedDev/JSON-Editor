...
class ModuleAdder {
	...
	parseTemplate(pObj, pModule, pModPath, pVars) {
		...
		} else if(typeof pObj != "function" && pObj) {
			console.log(new StringStatement(pObj, this.path, pModule, pModPath, pVars).execute());
...
class StringStatement {
	...
	execute() {
		...
		if(arr.length == 1 & arr[0].split("(")[0] == "$module") {
...
class ModuleStatement extends FunctionStatement {
	...
	$var(pArg) {
		...
		if(!variable && this.vars[pArg]) {
			variable = this.vars[pArg];
		}
		return variable ? this.parseTypes(variable) : "-";
	}
	$small_uuid() {
		var d = new Date().getTime();
		var uuid = 'xxxxxxxx-xxxx'.replace(/[xy]/g, function(pC) {
